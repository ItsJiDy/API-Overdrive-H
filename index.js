...
App.post('/v1/executions', (Request, Response) => {
    ...
    if (api_key) {
        ...
        if (key) {
            const req = Https.get(get_api_key + key, (Res) => {
                let res = ''
                Res.on('data', (Chunk) => {
                    res += Chunk
                })
        ...
        if (key) {
            const req = Https.get(get_api_key + key, (Res) => {
                ...
                Res.on('end', () => {
                    res = JSON.parse(res)
                    if (!res.errors) {
                        const req2 = Https.get('https://inventory.roblox.com/v1/users/' + res.id + '/items/GamePass/22739804', (Res) => {
                            let Data = ''
                            Res.on('data', (Chunk) => {
                                Data += Chunk
                            })
        ...
        if (key) {
            const req = Https.get(get_api_key + key, (Res) => {
                ...
                Res.on('end', () => {
                    res = JSON.parse(res)
                    if (!res.errors) {
                        const req2 = Https.get('https://inventory.roblox.com/v1/users/' + res.id + '/items/GamePass/926084952', (Res) => {
                            let Data = ''
                            Res.on('data', (Chunk) => {
                                Data += Chunk
                            })
        ...
        if (key) {
            const req = Https.get(get_api_key + key, (Res) => {
                ...
                Res.on('end', () => {
                    ...
                    if (!res.errors) {
                        const userId = res.id
                        const req2 = Https.get("https://" + firebase + "/bans.json", (Res) => {
                            let res = ''
                            Res.on('data', (Chunk) => {
                                res += Chunk
                            })
